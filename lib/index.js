      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipStack.length && (match = skipStack[skipStack.length - 1] || (line === 'endif::[]' && line))) {
          ...
          if ((grab = DELIMS[line]) === 'v') {
            ...
            if (containerStack.length && containerStack[containerStack.length - 1].isTabsContainer && containerStack[containerStack.length - 1].currentTabTitle) {
              const tabTitle = containerStack[containerStack.length - 1].currentTabTitle
              line = style ? '```' + style + ' {{ title: \'' + tabTitle + '\' }}' : '```' + ' {{ title: \'' + tabTitle + '\' }}'
              inContainer.cap = '```'

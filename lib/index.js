      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipStack.length && (match = skipStack[skipStack.length - 1] || (line === 'endif::[]' && line))) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            // Drop a preceding `<a id="X"/>` anchor from an immediately-prior
            // `[[parent__X]]` when X matches this heading's slug. Rehype-slug
            // will assign the same id to the heading itself, so cross-refs
            // like `<<parent__X>>` → `/parent#X` still resolve. Leaving the
            // anchor creates a duplicate id + displaces the heading from
            // :first-child (breaking prose margin reset).
            {
              let i = accum.length
              while (i > 0 && accum[i - 1] === '') i--
              const prev = i > 0 ? accum[i - 1] : undefined
              const anchorMatch = prev && /^\s*<a id="([^"]+)"\/>\s*$/.exec(prev)
              if (anchorMatch && anchorMatch[1] === autoId) {
                accum.splice(i - 1, accum.length - (i - 1))
              }
            }
